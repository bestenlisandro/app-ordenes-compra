...
const Info = ({ label, value, children }) => <div className="supplier-info"><span>{label}</span>{children || <strong>{value || '—'}</strong>}</div>;
...
export default function Suppliers() {
  ...
  return <div className="grid gap-6">
    ...
    <section className="suppliers-list"><div className="list-heading"><div><h2>Proveedores registrados</h2><p>{suppliers.length} proveedor{suppliers.length===1?'':'es'} · información completa</p></div></div><div className="supplier-records">{suppliers.map(s=><article key={s.id} className="supplier-record">
      <header><div><h3>{s.nombre}</h3><p>{s.razonSocial||'Sin razón social'} · CUIT {s.taxId}</p></div><div className="supplier-card-actions"><button onClick={()=>edit(s)}>Editar</button><button onClick={()=>remove(s.id)} className="text-red-600" aria-label={`Eliminar ${s.nombre}`}><Trash2 size={17}/></button></div></header>
      <div className="supplier-detail-section"><h4>Identificación legal y fiscal</h4><div className="supplier-detail-grid"><Info label="Razón social" value={s.razonSocial}/><Info label="Nombre comercial" value={s.nombre}/><Info label="CUIT" value={s.taxId}/><Info label="Condición frente al IVA" value={s.condicionIva}/><Info label="Domicilio fiscal" value={[s.direccion,s.numero,s.piso,s.ciudad,s.provincia,s.codigoPostal,s.pais].filter(Boolean).join(', ')}/></div></div>
      <div className="supplier-detail-section"><h4>Contacto</h4><div className="supplier-detail-grid"><Info label="Persona de contacto" value={s.contacto}/><Info label="Teléfono" value={s.telefono}/><Info label="Email" value={s.email}/><Info label="Sitio web">{s.sitioWeb?<a href={s.sitioWeb} target="_blank" rel="noreferrer">{s.sitioWeb}</a>:<strong>—</strong>}</Info><Info label="Horario de atención" value={s.horarioAtencion}/></div></div>
      <div className="supplier-detail-section"><h4>Datos comerciales y financieros</h4><div className="supplier-detail-grid"><Info label="Condición de pago" value={s.condicionPago}/><Info label="Moneda" value={s.moneda||'ARS'}/><Info label="Lista de precios" value={s.listaPrecios}/><Info label="Descuento por volumen" value={s.descuentoVolumen}/><Info label="Costo / condición de envío" value={s.costoEnvio}/><Info label="Plazo de entrega" value={s.tiempoEntrega==null?'—':`${s.tiempoEntrega} días`}/></div></div>
      <div className="supplier-detail-section"><h4>Datos avanzados</h4><div className="supplier-detail-grid"><Info label="Calificación" value={s.calificacion?`${'★'.repeat(s.calificacion)} (${s.calificacion}/5)`:'Sin calificar'}/><Info label="Categoría de productos" value={s.categoriaProductos}/><Info label="Notas internas" value={s.notasInternas}/><Info label="Archivos adjuntos">{s.archivos?.length?<div className="supplier-downloads">{s.archivos.map((a,i)=><a key={`${a.nombre}-${i}`} href={a.datos} download={a.nombre}><Download size={13}/>{a.nombre}</a>)}</div>:<strong>Sin archivos</strong>}</Info></div></div>
