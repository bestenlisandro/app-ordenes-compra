...
export default function OrderForm({ onCancel, onCreated }) {
  ...
  const selectSupplier = (id) => {
    if (Number(id) !== selectedSupplier) {
      const hadSelections = rows.some((row) => !!row.productoId);
  ...
  const submit = async (values) => { setApiError(''); if (values.items.some((row) => !supplierProducts.some((product) => product.id === Number(row.productoId)))) { setApiError('Todos los materiales deben estar asociados al proveedor de la orden. Revisá los ítems seleccionados.'); return; } try { const response = await fetch('/api/orders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(values) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error); onCreated(payload); } catch (error) { setApiError(error.message || 'No se pudo crear la orden.'); } };
