...
import { materialsForSupplier } from '../utils/supplierMaterials.mjs';

const optionalText = z.string().max(2000, 'Máximo 2000 caracteres.').optional();
const orderItemSchema = z.object({
  tipo: z.enum(['CATALOGO', 'LIBRE']),
  productoId: z.union([z.coerce.number().int().positive(), z.literal('')]).optional(),
  codigoLibre: z.string().max(100, 'Máximo 100 caracteres.').optional(),
  descripcionLibre: z.string().max(500, 'Máximo 500 caracteres.').optional(),
  cantidad: z.coerce.number().positive('Debe ser mayor a cero.'),
  precioUnitario: z.coerce.number().positive('Debe ser mayor a cero.'),
}).superRefine((item, context) => {
  if (item.tipo === 'LIBRE' && !item.descripcionLibre?.trim()) context.addIssue({ code: z.ZodIssueCode.custom, path: ['descripcionLibre'], message: 'Escriba la descripción del ítem.' });
  if (item.tipo === 'CATALOGO' && !Number(item.productoId)) context.addIssue({ code: z.ZodIssueCode.custom, path: ['productoId'], message: 'Seleccione un material.' });
});
const schema = z.object({
  ...
  items: z.array(orderItemSchema).min(1, 'Agregue al menos un ítem.'),
});
...
const defaults = {
  ...
  items: [{ tipo: 'CATALOGO', productoId: '', codigoLibre: '', descripcionLibre: '', cantidad: 1, precioUnitario: 0 }],
};
...
export default function OrderForm({ onCancel, onCreated }) {
  ...
  const supplierProducts = useMemo(() => materialsForSupplier(products, selectedSupplier), [products, selectedSupplier]);
  ...
  const selectSupplier = (id) => {
    if (Number(id) !== selectedSupplier) {
      const hadSelections = rows.some((row) => row.tipo !== 'LIBRE' && !!row.productoId);
      rows.forEach((row, index) => {
        if (row.tipo !== 'LIBRE') {
          setValue(`items.${index}.productoId`, '', { shouldDirty: true });
          setValue(`items.${index}.precioUnitario`, 0, { shouldDirty: true });
        }
      });
      setSupplierNotice(hadSelections ? 'Cambiaste el proveedor. Volvé a elegir los materiales para cargar sus precios y equivalencias; las cantidades se conservaron.' : '');
    }
    const supplier = suppliers.find((item) => item.id === Number(id)); if (!supplier) { ['proveedorRazonSocial', 'proveedorTaxId', 'proveedorContacto', 'proveedorDireccion', 'proveedorDatosContacto'].forEach((name) => setValue(name, '')); return; } setValue('proveedorRazonSocial', supplier.razonSocial || supplier.nombre); setValue('proveedorTaxId', supplier.taxId); setValue('proveedorContacto', supplier.contacto || ''); setValue('proveedorDireccion', [supplier.direccion, supplier.ciudad, supplier.provincia, supplier.pais].filter(Boolean).join(', ')); setValue('proveedorDatosContacto', [supplier.email, supplier.telefono].filter(Boolean).join(' · ')); };
  const pickProduct = (index, productId) => { const product = supplierProducts.find((item) => item.id === Number(productId)); if (product) { const offer = product.ofertas?.find((itemOffer) => itemOffer.proveedorId === selectedSupplier); setValue(`items.${index}.precioUnitario`, Number(offer?.precioSinIva ?? product.precioUnitario), { shouldValidate: true }); if (offer?.moneda) setValue('moneda', offer.moneda); } };
  const submit = async (values) => { setApiError(''); if (values.items.some((row) => row.tipo === 'CATALOGO' && !supplierProducts.some((product) => product.id === Number(row.productoId)))) { setApiError('Los materiales registrados deben estar asociados al proveedor de la orden. Revisá los ítems seleccionados.'); return; } try { const response = await fetch('/api/orders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(values) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.error); onCreated(payload); } catch (error) { setApiError(error.message || 'No se pudo crear la orden.'); } };
  ...
  return <form onSubmit={handleSubmit(submit)} className="space-y-6">
    ...
    {supplierNotice && <p role="status" className="rounded-lg bg-orange-50 p-3 text-sm text-orange-800">{supplierNotice}</p>}
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b p-5"><div><h2 className="text-lg font-semibold">Ítems de la orden</h2><p className="mt-1 text-xs text-slate-500">Elegí un material asociado o escribí un ítem libre para este proveedor.</p></div><button type="button" onClick={() => append({ tipo: 'CATALOGO', productoId: '', codigoLibre: '', descripcionLibre: '', cantidad: 1, precioUnitario: 0 })} className="btn-secondary py-2"><Plus size={16}/> Agregar ítem</button></div><div className="overflow-x-auto"><table className="w-full min-w-[820px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="p-3">Material / descripción libre</th><th className="p-3">Cantidad</th><th className="p-3">Precio unitario</th><th className="p-3 text-right">Subtotal</th><th /></tr></thead><tbody>{fields.map((field, index) => { const row = rows?.[index] || {}; const isFree = row.tipo === 'LIBRE'; const lineTotal = (Number(row.cantidad) || 0) * (Number(row.precioUnitario) || 0); const selectedProduct = products.find((product) => product.id === Number(row.productoId)); const equivalence = selectedProduct?.ofertas?.find((offer) => offer.proveedorId === selectedSupplier); return <tr key={field.id} className="border-t align-top"><td className="p-3"><div className="mb-2 flex gap-2"><button type="button" className={isFree?'btn-secondary py-1.5 opacity-60':'btn-primary py-1.5'} onClick={() => setValue(`items.${index}.tipo`, 'CATALOGO', { shouldValidate: true, shouldDirty: true })}>Material registrado</button><button type="button" className={isFree?'btn-primary py-1.5':'btn-secondary py-1.5 opacity-60'} onClick={() => { setValue(`items.${index}.tipo`, 'LIBRE', { shouldValidate: true, shouldDirty: true }); setValue(`items.${index}.productoId`, '', { shouldDirty: true }); setValue(`items.${index}.precioUnitario`, 0, { shouldDirty: true }); }}>Ítem libre</button></div><input type="hidden" {...register(`items.${index}.tipo`)} />{isFree ? <div className="grid gap-2 sm:grid-cols-[150px_1fr]"><div><input {...register(`items.${index}.codigoLibre`)} className="field" placeholder="Código (opcional)" /></div><div><input {...register(`items.${index}.descripcionLibre`)} className="field" placeholder="Descripción, por ejemplo: Clavo 2 pulgadas" />{errors.items?.[index]?.descripcionLibre && <p className="mt-1 text-xs text-red-600">{errors.items[index].descripcionLibre.message}</p>}</div></div> : <><input type="hidden" {...register(`items.${index}.productoId`)} /><OrderMaterialSearch key={selectedSupplier} products={supplierProducts} value={row.productoId} supplierId={selectedSupplier} itemNumber={index + 1} invalid={!!errors.items?.[index]?.productoId} onSelect={(productId) => { setValue(`items.${index}.productoId`, productId, { shouldValidate: true, shouldDirty: true }); if (productId) pickProduct(index, productId); else setValue(`items.${index}.precioUnitario`, 0, { shouldDirty: true }); }} />{equivalence && <p className="mt-1.5 rounded-lg bg-orange-50 px-2.5 py-1.5 text-xs text-orange-800"><strong>En la OC:</strong> {equivalence.codigoProveedor || selectedProduct.codigo} — {equivalence.nombreProveedor || selectedProduct.descripcion}</p>}{errors.items?.[index]?.productoId && <p className="mt-1 text-xs text-red-600">{errors.items[index].productoId.message}</p>}</>}</td><td className="p-3"><input type="number" min="0.01" step="0.01" {...register(`items.${index}.cantidad`)} className="field" />{errors.items?.[index]?.cantidad && <p className="mt-1 text-xs text-red-600">{errors.items[index].cantidad.message}</p>}</td><td className="p-3"><input type="number" min="0.01" step="0.01" {...register(`items.${index}.precioUnitario`)} className="field" />{errors.items?.[index]?.precioUnitario && <p className="mt-1 text-xs text-red-600">{errors.items[index].precioUnitario.message}</p>}</td><td className="p-3 pt-5 text-right font-medium">{money(lineTotal)}</td><td className="p-3"><button type="button" aria-label="Eliminar ítem" disabled={fields.length === 1} onClick={() => remove(index)} className="rounded-md p-2 text-red-600 disabled:opacity-30"><Trash2 size={18}/></button></td></tr>; })}</tbody></table></div>
