...
function MaterialSearch({ products, value, onSelect, supplierId }) {
  ...
  useEffect(() => { if (selected) setQuery(`${selected.codigo} — ${selected.descripcion}`); }, [value, selected?.codigo, selected?.descripcion]);
  const normalized = query.trim().toLocaleLowerCase('es');
  const matches = products.filter((product) => {
    const offer = product.ofertas?.find((item) => item.proveedorId === supplierId);
    return !normalized || [product.codigo, product.descripcion, product.marca, offer?.codigoProveedor, offer?.nombreProveedor].filter(Boolean).some((text) => text.toLocaleLowerCase('es').includes(normalized));
  }).slice(0, 8);
  return <div className="material-search"><Search size={17}/><input value={query} onFocus={() => setOpen(true)} onBlur={() => setTimeout(() => setOpen(false), 150)} onChange={(event) => { setQuery(event.target.value); setOpen(true); if (value) onSelect(''); }} placeholder="Escribí código o nombre del material…" autoComplete="off" />{open && <div className="material-suggestions" role="listbox">{matches.length ? matches.map((product) => { const offer = product.ofertas?.find((item) => item.proveedorId === supplierId); return <button type="button" role="option" aria-selected={product.id === Number(value)} key={product.id} onMouseDown={(event) => event.preventDefault()} onClick={() => { onSelect(product.id); setOpen(false); }}><span><strong>{product.codigo}</strong> — {product.descripcion}<small>{offer ? `Proveedor: ${offer.codigoProveedor || product.codigo} — ${offer.nombreProveedor || product.descripcion}` : [product.marca, product.categoria].filter(Boolean).join(' · ')}</small></span>{product.id === Number(value) && <Check size={16}/>}</button>; }) : <p>No hay materiales que coincidan.</p>}</div>}</div>;
}
